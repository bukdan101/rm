...
async function seed() {
  ...
  // Insert Models - without onConflict
  console.log('\n📥 Inserting models...')
  ...
  for (const brand of insertedBrands || []) {
    ...
    const { data, error } = await admin.from('car_models').insert(toInsert).select()
    if (error) {
      // If error, try to get existing models
      const { data: existing } = await admin.from('car_models').select('*').eq('brand_id', brand.id)
      if (existing) allModels.push(...existing)
      console.log(`  ⚠️ ${brand.name}: ${error.message.substring(0, 50)}, using ${existing?.length || 0} existing`)
    }
    if (data) {
      allModels.push(...data)
      console.log(`  ✅ ${brand.name}: ${data.length} models inserted`)
    }
  }
