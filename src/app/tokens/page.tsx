...
export default function TokensPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('/api/credit-packages?active=true');
        if (response.ok) {
          const data = await response.json();
          // Normalize package data
          const packages = (data.packages || []).map((pkg: CreditPackage) => ({
            ...pkg,
            isActive: pkg.is_active ?? pkg.isActive ?? true,
            bonusCredits: pkg.bonus_credits ?? pkg.bonusCredits ?? 0,
            sortOrder: pkg.sort_order ?? pkg.sortOrder ?? 0
          }));
    ...
    try {
      ...
      if (response.ok && data.success) {
        if (paymentMethod === 'online') {
          ...
          // Refresh credits
          setUserCredits(prev => (prev || 0) + data.creditsAdded);
        } else {
          toast.info(`Instruksi transfer untuk ${packageName}`, {
            description: 'Silakan transfer ke rekening BNI: 1234567890 a.n. AutoMarket Indonesia',
          });
        }
      } else {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold mb-4">Paket Kredit AutoMarket</h1>
        ...
        {loadingPackages ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {creditPackages.map((pkg, index) => {
              const Icon = packageIcons[index % 4] || CreditCard;
              const colors = packageColors[index % 4] || packageColors[0];
              const isPopular = index === 1; // Second package is popular
              const bonusCredits = pkg.bonusCredits || pkg.bonus_credits || 0;
                  ...
                  <CardContent>
                    <div className="space-y-2 mb-4">
                      ...
                      {bonusCredits > 0 && (
                        <div className="flex items-start gap-2">
                          <Check className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                          <span className="text-sm">Bonus {bonusCredits} kredit</span>
                        </div>
                      )}
                      {index >= 1 && (
                        <div className="flex items-start gap-2">
                    ...
                    {bonusCredits > 0 && (
                      <div className="text-sm text-muted-foreground">
                        Hemat {Math.round((bonusCredits / (pkg.credits + bonusCredits)) * 100)}%
                      </div>
                    )}
                  </CardContent>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Apa itu kredit AutoMarket?</CardTitle>
              </CardHeader>
              <CardContent>
