...
export function TokenWidget({ compact = false, showActions = true }: TokenWidgetProps) {
    ...
    <Card className="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 dark:from-amber-950/30 dark:via-orange-950/30 dark:to-yellow-950/30 border-amber-200 dark:border-amber-800 overflow-hidden relative">
      ...
      <CardContent className="p-5 relative z-10">
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-sm text-amber-700 dark:text-amber-300 font-medium mb-1">Saldo Token Anda</p>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              ≈ Rp {((balance || 0) * 10000).toLocaleString('id-ID')} (1 token = Rp 10.000)
            </p>
          </div>
        ...
        {showActions && (
          <div className="flex gap-2">
            <Link href="/dashboard/tokens" className="flex-1">
              <Button className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white">
                <Plus className="h-4 w-4 mr-2" />
                Beli Token
              </Button>
            </Link>
            <Link href="/dashboard/tokens?tab=history">
        ...
        <div className="mt-4 pt-4 border-t border-amber-200 dark:border-amber-800">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Marketplace: 3-5 Token • Inspeksi: 10 Token</span>
          </div>
        </div>
      </CardContent>
    </Card>
...
// Mini token display for header/sidebar
