...
-- Semua tabel yang diperlukan untuk Admin Dashboard
-- =====================================================
...
CREATE TABLE IF NOT EXISTS kyc_verifications (
    ...
    id_card_url TEXT,
    selfie_url TEXT,
    document_type VARCHAR(20) DEFAULT 'ktp', -- ktp, passport, sim
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    type VARCHAR(30) NOT NULL CHECK (type IN (
        'token_purchase', 'listing_fee', 'feature_fee', 'subscription',
        'withdrawal', 'refund', 'topup', 'payment', 'commission'
    )),
    ...
    -- Reference
    reference_type VARCHAR(50), -- listing, token_package, withdrawal, etc
    reference_id UUID,
    
    -- Payment Info
    payment_method VARCHAR(50), -- bank_transfer, e_wallet, credit_card
    payment_provider VARCHAR(50), -- midtrans, xendit, doku
...
CREATE TABLE IF NOT EXISTS topup_requests (
    ...
    -- Amount
    amount DECIMAL(15,2) NOT NULL,
    tokens INTEGER, -- Jumlah token yang akan didapat
...
CREATE TABLE IF NOT EXISTS reports (
    ...
    -- Reported Content
    report_type VARCHAR(30) NOT NULL CHECK (report_type IN ('listing', 'user', 'dealer', 'message', 'review')),
    reported_user_id UUID REFERENCES profiles(id) ON DELETE SET NULL,
    reported_dealer_id UUID REFERENCES dealers(id) ON DELETE SET NULL,
    listing_id UUID REFERENCES car_listings(id) ON DELETE SET NULL,
    
    -- Report Details
    category VARCHAR(50) NOT NULL, -- spam, fraud, inappropriate, fake, other
    ...
    action_taken VARCHAR(50), -- warning, suspended, banned, listing_removed, dismissed
...
CREATE TABLE IF NOT EXISTS support_tickets (
    ...
    category VARCHAR(50), -- technical, billing, account, listing, other
    priority VARCHAR(10) DEFAULT 'normal' CHECK (priority IN ('low', 'normal', 'high', 'urgent')),
...
CREATE TABLE IF NOT EXISTS banners (
    ...
    -- Position
    position VARCHAR(30) DEFAULT 'home_hero' CHECK (position IN (
        'home_hero', 'home_middle', 'home_bottom', 
        'marketplace_top', 'marketplace_sidebar',
        'listing_detail', 'custom'
    )),
    display_order INTEGER DEFAULT 0,
    
    -- Targeting
    target_user VARCHAR(20) DEFAULT 'all' CHECK (target_user IN ('all', 'user', 'dealer', 'guest')),
...
CREATE TABLE IF NOT EXISTS coupons (
    ...
    -- Discount
    discount_type VARCHAR(20) NOT NULL CHECK (discount_type IN ('percentage', 'fixed', 'tokens')),
    ...
    -- Applicability
    applies_to VARCHAR(30) DEFAULT 'all' CHECK (applies_to IN ('all', 'token_purchase', 'listing', 'subscription')),
    target_user VARCHAR(20) DEFAULT 'all' CHECK (target_user IN ('all', 'user', 'dealer')),
...
CREATE TABLE IF NOT EXISTS broadcasts (
    ...
    -- Type
    broadcast_type VARCHAR(30) DEFAULT 'notification' CHECK (broadcast_type IN (
        'notification', 'email', 'sms', 'whatsapp', 'push'
    )),
    
    -- Target
    target_audience VARCHAR(30) DEFAULT 'all' CHECK (target_audience IN (
        'all', 'users', 'dealers', 'sellers', 'buyers', 'specific'
    )),
...
CREATE TABLE IF NOT EXISTS activity_logs (
    ...
    -- Action
    action VARCHAR(50) NOT NULL, -- login, logout, create_listing, update_listing, etc
    entity_type VARCHAR(50), -- user, listing, transaction, etc
    entity_id UUID,
...
CREATE TABLE IF NOT EXISTS boost_settings (
    ...
    -- Boost Type
    boost_type VARCHAR(30) NOT NULL CHECK (boost_type IN (
        'featured', 'highlight', 'top_search', 'premium_badge', 'urgent'
    )),
    name VARCHAR(100) NOT NULL,
    description TEXT,
...
BEGIN
    ...
    LOOP
        EXECUTE format('CREATE TRIGGER update_%s_updated_at BEFORE UPDATE ON %s FOR EACH ROW EXECUTE FUNCTION update_admin_tables_updated_at()', t, t);
    END LOOP;
END;
$$;
